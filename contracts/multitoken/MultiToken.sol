...
import "contracts/multitoken/interfaces/ICryptoKitties.sol";
...
library MultiToken {
    ...
    function _transferAssetFrom(
        ...
        if (asset.category == Category.ERC20) {
            ...
            if (source == address(this))
                ...
                ICryptoKitties(asset.assetAddress).transferFrom(
                    source,
                    dest,
                    asset.id
                );
    ...
    function getTransferAmount(
        ...
        if (asset.category == Category.ERC20) return asset.amount;
        else if (asset.category == Category.ERC1155 && asset.amount > 0)
            return asset.amount;
        // Return 1 for ERC721, CryptoKitties and ERC1155 used as NFTs (amount = 0)
        else return 1;
    }
    ...
    function transferAssetFromCalldata(
        ...
        return
            _transferAssetFromCalldata(asset, source, dest, fromSender, false);
    }
     ...
     * @param dest Destination address.
     */
    function safeTransferAssetFromCalldata(
        Asset memory asset,
        address source,
        address dest,
        bool fromSender
        ...
        return
            _transferAssetFromCalldata(asset, source, dest, fromSender, true);
    }
    ...
    function balanceOf(
        ...
        if (asset.category == Category.ERC20) {
            ...
            return
                IERC721(asset.assetAddress).ownerOf(asset.id) == target ? 1 : 0;
        } else if (asset.category == Category.ERC1155) {
            return IERC1155(asset.assetAddress).balanceOf(target, asset.id);
        } else if (asset.category == Category.CryptoKitties) {
            return
                ICryptoKitties(asset.assetAddress).ownerOf(asset.id) == target
                    ? 1
                    : 0;
        } else {
            revert("MultiToken: Unsupported category");
    ...
    function isValid(Asset memory asset) internal view returns (bool) {
        if (asset.category == Category.ERC20) {
            ...
            if (asset.assetAddress.supportsERC165()) {
                ...
                return
                    asset.assetAddress.supportsERC165InterfaceUnchecked(
                        ERC20_INTERFACE_ID
                    );
            } else {
            ...
            return
                asset.assetAddress.supportsInterface(
