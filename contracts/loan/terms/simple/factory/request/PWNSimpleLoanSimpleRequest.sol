...
import "contracts/loan/lib/PWNSignatureChecker.sol";
import "contracts/loan/terms/simple/factory/request/base/PWNSimpleLoanRequest.sol";
...
contract PWNSimpleLoanSimpleRequest is PWNSimpleLoanRequest {
    ...
    function createLOANTerms(
        ...
        override
        onlyActiveLoan
        returns (PWNLOANTerms.Simple memory loanTerms)
    {
        ...
        if (requestsMade[requestHash] == false)
            if (
                PWNSignatureChecker.isValidSignatureNow(
                    borrower,
                    requestHash,
                    signature
                ) == false
        ...
        if (request.duration < MIN_LOAN_DURATION) revert InvalidDuration();
        ...
        return
            keccak256(
                abi.encodePacked(
                    ...
                    DOMAIN_SEPARATOR,
                    keccak256(
                        abi.encodePacked(REQUEST_TYPEHASH, abi.encode(request))
                    )
                )
            );
    }
