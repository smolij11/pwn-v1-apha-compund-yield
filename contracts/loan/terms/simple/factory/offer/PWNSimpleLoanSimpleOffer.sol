...
import "contracts/loan/terms/simple/factory/offer/base/PWNSimpleLoanOffer.sol";
...
contract PWNSimpleLoanSimpleOffer is PWNSimpleLoanOffer {
        ...
        returns (PWNLOANTerms.Simple memory loanTerms)
    {
        ...
        if (offer.duration < MIN_LOAN_DURATION) revert InvalidDuration();
        ...
        return
            keccak256(
                abi.encodePacked(
                    ...
                    DOMAIN_SEPARATOR,
                    keccak256(
                        abi.encodePacked(OFFER_TYPEHASH, abi.encode(offer))
                    )
                )
            );
    }
