...
import "contracts/hub/PWNHubAccessControl.sol";
import "contracts/loan/token/IERC5646.sol";
import "contracts/loan/token/IPWNLoanMetadataProvider.sol";
...
contract PWNLOAN is PWNHubAccessControl, IERC5646, ERC721 {
    ...
    constructor(
        address hub
    ) PWNHubAccessControl(hub) ERC721("PWN LOAN", "LOAN") {}
    ...
    function mint(
        address owner
    ) external onlyActiveLoan returns (uint256 loanId) {
        loanId = ++lastLoanId;
    ...
    function tokenURI(
        ...
        return
            IPWNLoanMetadataProvider(loanContract[tokenId]).loanMetadataUri();
    }
